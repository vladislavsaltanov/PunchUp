...
public class GuardionAudio : MonoBehaviour
{
    [Header("References")]
    ...
    private void HandleFootsteps()
    {
        ...
        if (stepTimer <= 0f)
        {
            ...
            AudioManager.Instance.PlayGuardianFootstep(transform.position);
            xPosLastFrame = transform.position.x;
    ...
    {
        AudioManager.Instance.GuardianTakeDamage(transform.position);
    }

    //Скидыщь
    public void HandleAttack()
    {
        AudioManager.Instance.GuardianAttack(transform.position);
    }
    public void HandleReload()
    {
        AudioManager.Instance.GuardianReload(transform.position);
    }
}
